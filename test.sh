...
test "request_trace_get" "$(dfx --identity default canister call core request_trace_get "(0:nat64)" 2>&1)" '( opt record { args = variant { token_deposit = record { arg = record { token = principal "ss2fx-dyaaa-aaaar-qacoq-cai"; from = record { owner = principal "' '"; subaccount = null; }; amount = 5_000_000_000_000_000_000 : nat; }; now = ' ' : nat64; created = null; memo = null; caller = principal "' '"; } }; done = opt record { ' ' : nat64; variant { Ok = "4" }; }; traces = vec { record { ' ' : nat64; "Token: [ss2fx-dyaaa-aaaar-qacoq-cai] Transfer height: 4."; }; }; locks = record { token = opt true; swap = null; balances = opt vec { record { token = principal "ss2fx-dyaaa-aaaar-qacoq-cai"; account = record { owner = principal "' '"; subaccount = null; }; }; }; }; index = 0 : nat64; }, )'
