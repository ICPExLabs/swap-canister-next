...
impl Business for InnerState {
    ...
    fn business_blocks_append(&mut self, blocks: Vec<EncodedBlock>) {
        ...
        ic_cdk::println!(
            "[swap archive node] append_blocks(): archive size: {} blocks, appending {} blocks",
            self.blocks.blocks_len(),
            blocks.len()
        );
        for block in &blocks {
            // 1. try to parse block bytes before append
            let token_block: proto::SwapBlock = trap(from_proto_bytes(&block.0));
        ...
        ic_cdk::println!(
            "[swap archive node] append_blocks(): done. archive size: {} blocks",
            self.blocks.blocks_len()
        );
    }
