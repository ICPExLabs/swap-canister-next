...
const WASM_MODULE: &[u8] = include_bytes!("../../../../../../archive-swap/sources/source_opt.wasm.gz");
...
impl SwapBlockChain {
    ...
    pub fn set_swap_current_archiving_max_length(&mut self, max_length: u64) -> Option<CurrentArchiving> {
        self.block_chain.set_current_archiving_max_length(max_length)
    }
    pub fn set_swap_archive_config(&mut self, archive_config: NextArchiveCanisterConfig) -> NextArchiveCanisterConfig {
        self.block_chain.set_archive_config(archive_config)
    }
    pub fn replace_swap_current_archiving(&mut self, archiving: CurrentArchiving) -> Option<CurrentArchiving> {
...
impl SwapBlockChainGuard<'_> {
    fn get_next_swap_block(
        ...
        let hash = block.do_hash().map_err(BusinessError::SwapBlockChainError)?;
