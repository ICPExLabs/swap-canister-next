...
pub(super) mod immutable {
        ...
        Pausable<PauseReason> + ParsePermission + Permissable<Permission> + Schedulable + ScheduleTask + StableHeap
    {
        ...
        fn business_token_blocks_get(
            &self,
            block_height: BlockIndex,
        ) -> Vec<(BlockIndex, QueryBlockResult<EncodedBlock>)> {
            ic_cdk::trap("Not supported operation by this version.")
        }
        fn business_swap_blocks_get(
            &self,
            block_height: BlockIndex,
        ) -> Vec<(BlockIndex, QueryBlockResult<EncodedBlock>)> {
            ic_cdk::trap("Not supported operation by this version.")
...
pub(super) mod mutable {
    ...
    pub trait MutableBusiness: Business {
        ...
        // token pairs
        fn business_token_pair_lock(
            &mut self,
            required: Vec<TokenPairAmm>,
        ) -> Result<TokenPairsLock, Vec<TokenPairAmm>> {
            ic_cdk::trap("Not supported operation by this version.")
        }
        ...
        fn business_maintain_pools(&mut self, self_canister: SelfCanister) -> Result<(), BusinessError> {
            ic_cdk::trap("Not supported operation by this version.")
        }
    }
    ...
    impl MutableBusiness for State {
        ...
        fn business_config_protocol_fee_replace(
            ...
            self.get_mut()
                .business_config_protocol_fee_replace(lock, pa, protocol_fee)
        }
        ...
        // set_certified_data
        fn business_certified_data_refresh(&mut self) {
            self.get_mut().business_certified_data_refresh()
        }
        ...
        fn business_token_pair_lock(
            ...
            self.get_mut().business_token_pair_lock(required)
        }
        fn business_token_pair_unlock(&mut self, locked: &HashSet<TokenPairAmm>) {
            self.get_mut().business_token_pair_unlock(locked)
        }
        ...
        fn business_token_pair_pool_remove(
            ...
            self.get_mut().business_token_pair_pool_remove(locks, arg)
        }
        // liquidity
        fn business_token_pair_liquidity_add(
            &mut self,
        ...
        fn business_maintain_pools(&mut self, self_canister: SelfCanister) -> Result<(), BusinessError> {
            self.get_mut().business_maintain_pools(self_canister)
        }
    }
}
