...
fn test_swap_business_fee_apis() {
  ...
  //assert_tokens_balance(    bob.tokens_balance_of(account(    bob_identity)).unwrap(),                                                                               vec![(token_sns_icx_canister_id, nat(             0)), (token_ck_eth_canister_id, nat(                        0)), (token_ck_usdt_canister_id, nat(              0))]);
    ...
    assert_token_block_chain_view(&default.config_token_block_chain_query(BlockChainArgs::BlockChainQuery).unwrap(), BlockChainView { current_archiving: Some(CurrentArchiving { canister_id: archive_token_canister_id, length: 33, max_length: 1_000_000, block_height_offset: 0 }), latest_block_hash: vec![].into(), archive_config: NextArchiveCanisterConfig { maintainers: Some(vec![default_identity]), max_memory_size_bytes: None, max_length: 1_000_000 }, next_block_index: 33, archived: vec![] });
    ...
    // 🚩 5 test archive-swap
    assert_swap_block_chain_view(&default.config_swap_block_chain_query(BlockChainArgs::BlockChainQuery).unwrap(), BlockChainView { current_archiving: Some(CurrentArchiving { canister_id: archive_swap_canister_id, length: 25, max_length: 1_000_000, block_height_offset: 0 }), latest_block_hash: vec![].into(), archive_config: NextArchiveCanisterConfig { maintainers: None, max_memory_size_bytes: None, max_length: 1_000_000 }, next_block_index: 25, archived: vec![] });
