...
const WASM_MODULE_1_0_8: &[u8] = include_bytes!("../sources/source_opt_1_0_8.wasm.gz");
...
fn test_swap_upgrade() {
    ...
    let arg: Vec<u8> = encode_one(None::<()>).unwrap();
    assert_eq!(arg, vec![68, 73, 68, 76, 1, 110, 127, 1, 0, 0]); // 4449444c016e7f010000
    pic.upgrade_canister(canister_id, WASM_MODULE_1_0_8.to_vec(), arg, Some(default_identity)).unwrap();
    default.pause_replace(None).unwrap();
